...
## 1. Kanıt
- Aktif hedef / kapsam: `backend/run_regression_tests.py`, `backend/output_manifest.py`, `backend/output_metrics.py` ve golden dataset sözleşmesinde çekirdek deterministik çıktı ile run-bağımlı gürültü sınırını netleştirmek; parser-backed determinism zinciri ise koruyucu regression olarak yeşil tutuluyor. Bu oturumda özel olarak topology health görünürlüğü ile bloklayıcı kabul otoritesi ayrımı netleştirildi.
- İlgili dosyalar:
...
- Kod kanıtı:
  ...
  - `backend/run_regression_tests.py`, topology health gate'i `required_status=HEALTHY` görünürlük eşiği olarak raporluyor; `enforced=false` ve `blocking_authority="topology_validator"` ile gerçek kabul/bloklama otoritesinin `TopologyValidator` olduğunu açıkça taşıyor.
...
- Sonuç:
  ...
  - WARNING topology health özeti regression çıktısında görünür kaldı; bloklayıcı hata yalnız validator FAIL olduğunda `Topology validation failed: ...` olarak gözlendi.
...
- Negatif senaryoların beklenen stderr/log çıktıları, özellikle PowerShell sarmalayıcı üzerinden okunduğunda yanlış başarısızlık izlenimi verebiliyor. Topology health görünürlüğü artık bloklayıcı kabul otoritesiyle karıştırılmıyor; ancak diğer beklenen negatif akışlarda benzer sinyal ayrımı henüz tam standart değil.
...
- `backend/run_regression_tests.py` içinde topology health gate görünürlük eşiği (`required_status=HEALTHY`) korunurken `enforced=false` ve `blocking_authority="topology_validator"` sözleşmesi eklendi; `_enforce_topology_health_gate(...)` legacy shim olarak bırakıldı ve bloklama `TopologyValidator.validate(...)` içinde tutuldu.
...
- Doğrulanan davranışlar:
  ...
  - WARNING topology health durumu suite sonucunu tek başına bozmadı; gerçek bloklayıcı kabul kararı `TopologyValidator` başarısızlıkları üzerinden raporlandı.
